...
/**
 * Avalanche Optimized Transaction Scanner
 * Uses Routescan/Snowtrace API to fetch transaction history.
 * Identifies contract creations and logs the resulting contract address.
 * 
 * Usage: node avalanche-scanner.js <network> <startHash> <limit>
 */
...
const NETWORKS = {
    fuji: {
        ...
        etherscanBase: "https://api.routescan.io/v2/network/testnet/evm/43113/etherscan/api",
        chainid: "43113",
        name: "Avalanche Fuji Testnet"
    },
    avalanche: {
        rpc: "https://api.avax.network/ext/bc/C/rpc",
        etherscanBase: "https://api.routescan.io/v2/network/mainnet/evm/43114/etherscan/api",
        chainid: "43114",
        name: "Avalanche Mainnet"
    }
};

const API_KEY = process.env.SNOWTRACE_API_KEY || process.env.ETHERSCAN_API_KEY || ""; // Routescan free tier allows 2 rps without a key
...
async function scan(networkKey, startHash, limit = 1000) {
    ...
    if (!config) {
        console.error(`Error: Unsupported network "${networkKey}". Use "fuji" or "avalanche".`);
        process.exit(1);
    }
    ...
    try {
        ...
        for (const tx of remainingTxs) {
            count++;
            // Routescan sets `to` = the created contract address (unlike Etherscan which sets it null/empty).
            // So we check contractAddress non-empty as the primary signal.
            const isCreation = (tx.contractAddress && tx.contractAddress !== "" && tx.contractAddress !== "0x0000000000000000000000000000000000000000")
...
if (!network || !hash) {
    console.log("Usage: node avalanche-scanner.js <fuji|avalanche> <txHash> [limit]");
    process.exit(1);
}
